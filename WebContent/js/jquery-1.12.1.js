/*
 * ! jQuery JavaScript Library v1.12.1 http://jquery.com/
 * 
...
jQuery.extend( {
	...
	when: function( subordinate /* , ..., subordinateN */ ) {
		var i = 0,
			...
			// the master Deferred.
			// If resolveValues consist of only a single Deferred, just use
			// that.
...
( function() {
	...
	// Support: IE<9 (lack submit/change bubble), Firefox (lack focus(in | out)
	// events)
	...
	simulate: function( type, elem, event ) {
		var e = jQuery.extend(
			...
			event,
			{
				...
				// jQuery.event.stopPropagation function we had a check for
				// existence of
				// originalEvent.stopPropagation method, so, consequently it
				// would be a noop.
				//
				// Guard for simulated events was moved to
				// jQuery.event.stopPropagation function
				// since `originalEvent` should point to the original event for
				// the
			...
			jQuery.event.add( this, "click._submit keypress._submit", function( e ) {
				...
				var elem = e.target,
					form = jQuery.nodeName( elem, "input" ) || jQuery.nodeName( elem, "button" ) ?
						...
						// to allow fixing the IE8 delegated submit issue
						// (gh-2332)
			...
			if ( forceAcceptData || acceptData( elem ) ) {
				...
				if ( data ) {
					...
					if ( cache[ id ] ) {
						...
						// IE does not have a removeAttribute function on
						// Document nodes
	...
	function( elem, computed ) {
		if ( computed ) {
			return (
				...
				// Support: IE<=11+
				// Running getBoundingClientRect on a disconnected node in IE
				// throws an error
				// Support: IE8 only
			...
			return {
				send: function( headers, complete ) {
					...
					if ( !options.async ) {
						...
						// Register the callback, but delay it in case
						// `xhr.send` throws
